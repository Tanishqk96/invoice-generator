...
const PricingCard = ({
  ...
  isAnnual = false,
  delay = 0,
  onCtaClick, // callback from parent
}) => (
  <div
    className={`
...
export default function Pricing() {
  ...
  // central handler for CTA clicks
  // second arg is optional control flags (used by SignedOut button to request sign-in)
  function handleCtaClick(planMeta, flags = {}) {
    // If caller explicitly requested sign-in UI or user not signed-in, open sign-in modal
    if (flags.openSignInFallback || !isSignedIn) {
      // prefer clerk.openSignIn() if available
      if (clerk && typeof clerk.openSignIn === "function") {
        // redirectAfterSignIn can be configured if you want the user to land on a route after sign-in
        clerk.openSignIn({ redirectUrl: "/app/create-invoice" });
      } else {
        // fallback: navigate to a hosted sign-in page (if you use one)
        navigate("/sign-in");
      }
      return;
    }

    // If user is signed in: take them where you want (create invoice / checkout / upgrade)
    // here we go to create-invoice as an example
    ...
    <section id="pricing" className={pricingStyles.section}>
      ...
      <div className={pricingStyles.container}>
        <div className={pricingStyles.headerContainer}>
          ...
          {/* Billing toggle + (optional) sign-in area can go here if you want */}
          <div
